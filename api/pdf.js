...
function buildPDF(doc, kit, meta, imgBuf) {
  ...
  const drawTableHeader = (doc, ty) => {
    doc.rect(ML, ty, CW, 20).fillColor(BLACK).fill();
    doc.fontSize(7).font('Helvetica-Bold').fillColor(WHITE);
    let cx = ML;
    [['ÍTEM','left'],['CANT.','center'],['UNIT. USD','right'],['TOTAL USD','right']].forEach(([h,a],i) => {
      doc.text(h, cx + tPad, ty + 6, { width: cw4[i] - tPad * 2, align: a, lineBreak: false, characterSpacing: 0.5 });
      cx += cw4[i];
    });
    return ty + 20;
  };

  let ty = drawTableHeader(doc, doc.y);
  ...
  if (!invFits) { renderInvestment(doc, inv, brand_name); doc.y += 18; }
